...
router.get("/:id/userhistory", isAccountOwner, (req, res) => {
    var id = req.params.id;
    var sql = "SELECT d.DELIVERED_DATE, d.DELIVERED_QUANTITY, g.DRUG_NAME, g.DRUG_PRICE, d.DELIVERED_QUANTITY * g.DRUG_PRICE as TOTAL_PRICE FROM delivered as d, drug_details as g, user_detail as u WHERE g.DRUG_ID = d.DRUG_ID AND u.USER_ID = d.USER_ID AND d.USER_ID = '" + id + "' ORDER BY d.DELIVERED_DATE ASC";
    connection.query(sql, (err, history, fields) => {
        if(err){
            console.log(err);
            req.flash("error", "Something went wrong");
            res.redirect('back');
            ...
            res.render("manage/users/history", {history: history, referer:req.headers.referer});
        }
    });
});
