    ...
    upload(req, res, (err) => {
        if(err){
            ...
            if(req.file == undefined){
                ...
                var data = [];
                data.push([display, name, Generic, image, type, price, pharma, Indications, Therapeutic, Pharmacology, Dosage, Overdose, Interaction, 
                    Contraindications, SideEffects, Pregnancy, Precautions, SpecialPopulations, Storage, addedBy]);

                var sql = "INSERT INTO drug_details (DRUG_FOR, DRUG_NAME, DRUG_GENERIC_NAME, DRUG_IMAGE, DRUG_TYPE, DRUG_PRICE, DRUG_PHARMA, DRUG_INDICATION,";
    ...
    upload(req, res, (err) => {
        if(err){
            ...
            if(req.file == undefined){
                ...
                var sql = 'UPDATE drug_details SET DRUG_NAME = "'+ name + '", DRUG_TYPE ="'+ type + '", DRUG_PRICE ="' + price + '", DRUG_FOR ="' + display;
                ...
                var sql = 'UPDATE drug_details SET DRUG_NAME = "'+ name + '", DRUG_IMAGE ="'+ image + '", DRUG_TYPE ="'+ type + '", DRUG_PRICE ="' + price + '", DRUG_FOR ="' + display;
                    sql += '", DRUG_GENERIC_NAME ="' + Generic + '", DRUG_PHARMACOLOGY ="' + Pharmacology + '", DRUG_DOSAGE="' + Dosage + '", DRUG_THERAPEUTIC_CLASS ="' + Therapeutic;
                    sql += '", DRUG_INTERACTION ="' + Interaction + '", DRUG_CONTRAINDICATIONS ="' + Contraindications + '", DRUG_SIDE_EFFECTS ="' + SideEffects;
                    sql += '", DRUG_PREGNANCY_AND_LACTATION ="' + Pregnancy + '", DRUG_PRECAUTIONS_AND_WARNINGS ="' + Precautions;
                    sql += '", DRUG_USE_IN_SPECIAL_POPULATION ="' + SpecialPopulations + '", DRUG_OVERDOSE_EFFECTS ="' + Overdose + '", DRUG_STORAGE_CONDITIONS ="' + Storage;
