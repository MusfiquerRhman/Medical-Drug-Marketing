...
router.get("/manage/orders/:orderID/edit", isLoggedIn, (req, res) => {
    var orderID = req.params.orderID;
    var sql = "SELECT c.ORDER_ID, d.DRUG_NAME, c.ORDER_QUANTITY FROM order_details AS C, drug_details AS d WHERE c.DRUG_ID = d.DRUG_ID AND ORDER_ID = '" + orderID + "'";
    connection.query(sql, (err, cart, fields) => {
        if(err){
            console.log(err);
            req.flash("error", "Something went wrong");
            ...
            res.render("manage/carts/editorder", {cart: cart});
        }
    });
});


router.post("/manage/orders/:orderID/edit", isLoggedIn, (req, res) => {
    var orderID = req.params.orderID;
    var Quantity = Number(req.body.Quantity);
    var sql = "UPDATE order_details SET ORDER_QUANTITY = " + Quantity +" WHERE ORDER_ID = '" + orderID + "'";
    connection.query(sql, (err, result, fields) => {
        if(err){
            console.log(err);
            req.flash("error", "Something went wrong");
...
router.get("/manage/orders/:orderID/delete", isLoggedIn, (req, res) => {
    var orderID = req.params.orderID;
